...
/** Teclado numérico fullscreen para desbloquear el POS con PIN de 4 dígitos. */
export function EmployeePINModal({ isValidating, error, onSubmit }: EmployeePINModalProps) {
  const [pin, setPin] = useState('')

  // Auto-enviar al completar los dígitos.
  useEffect(() => {
    if (pin.length === LIMITS.pos.pinLength) {
      onSubmit(pin)
      setPin('')
    }
  }, [pin, onSubmit])
  ...
  function handleDigit(digit: string) {
    ...
    setPin((prev) => (prev.length < LIMITS.pos.pinLength ? prev + digit : prev))
  }
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 px-6">
      ...
      <div className="flex items-center gap-3" aria-label="PIN">
        {Array.from({ length: LIMITS.pos.pinLength }, (_, i) => (
          <span
            key={i}
